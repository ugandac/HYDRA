    ...
    conn.sendMessage(m.chat, {image: {url: yt_play[0].thumbnail}, caption: texto1}, {quoted: m});
     if (command == 'play') {
      try {
        ...
        try {
          const lolhuman = await fetch(`https://globaltech-api.onrender.com/api/audio/yt?apikey=${yt_play[0].url}`);
...
async function ytMp3(url) {
  return new Promise((resolve, reject) => {
    ytdl.getInfo(url).then(async (getUrl) => {
      ...
      const tiny = await axios.get(`https://globaltech-api.onrender.com/api/audio/yt?apikey=${resultFix[0].audio}`);
...
async function ytMp4(url) {
  return new Promise(async (resolve, reject) => {
    ytdl.getInfo(url).then(async (getUrl) => {
      ...
      const tiny = await axios.get(`https://globaltech-api.onrender.com/api/audio/yt?apikey=${resultFix[0].video}`);
      const tinyUrl = tiny.data;
      const title = getUrl.videoDetails.title;
      const thumb = getUrl.player_response.microformat.playerMicroformatRenderer.thumbnail.thumbnails[0].url;
